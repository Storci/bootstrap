...
common.cardLineTrend('IDDivChart1', entityName, 'Pasta_Instant_PV_Temperatura_Camera', '', '°C')
// Grafico card Velocità Motore
common.cardLineTrend('IDDivChart2', entityName, 'Pasta_Instant_PV_Portata_Vapore', '', 'm3/h')
...
async function setLineinfo(entityName){
  ...
  let line_info = await tw.getLinePastaInstantInfo(entityName);
	...
	$("#cooking_time").text(line_info.Pasta_Instant_Ricetta_Tempo_Cottura);
	try{ $("#room_temp").text(line_info.Pasta_Instant_PV_Temperatura_Camera.toFixed(2)+ "  °C") }catch(e){}
	try{ $("#steam_flow").text(line_info.Pasta_Instant_PV_Portata_Vapore.toFixed(2)+ " kg/h") }catch(e){}
